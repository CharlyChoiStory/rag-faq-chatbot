...
## 11. 현재 추가 목표: 생성형 AI PoC

기존 사내 규정 RAG 챗봇을 유지하면서 기본 생성형 AI 기능을 추가한다. Codex는 코드 수정 전에 반드시 `GENERATIVE_AI_POC_PLAN.md`를 읽어야 한다.
...
3. 규정 기반 공지문 작성
4. 회의록/업무 메모 요약
5. 사내 정책·업무 개선 아이디어 생성

구현 원칙:

- 빠른 PoC는 기존 API에 `mode`를 추가하는 방식으로 진행한다.
- mode 값은 `qa | email | notice | meeting-summary | ideation`을 사용한다.
- 규정 관련 사실은 RAG 검색 근거를 따른다.
- 이메일/공지문/아이디어의 문체와 구성은 생성 가능하다.
- 회의록 요약은 입력 내용에 없는 사실을 만들지 않는다.
